...
def get_current_version(script_id: str) -> Tuple[int, int]:
    ...
    try:
        cur.execute(
            """SELECT `version`, `release` FROM apply_history
                WHERE script_id = %s
...
def update_version(script_info: ScriptInfo):
    ...
    try:
        cur.execute(
            """INSERT INTO apply_history(`script_id`, `version`, `release`)
            values (%s, %s, %s)
        """,
...
def apply_script(dirname: str, script_id: str, scripts: List[ScriptInfo]):
    ...
    for script in scripts:
        ...
        if version < current_version or (
            version == current_version and release == current_release
        ):
            print("Skiped", script_id, version, release)
            continue
        if version == current_version:
            undo_script = "{}.{:03d}.{:02d}.undo.sql".format(
                script_id, current_version, current_release
            )
